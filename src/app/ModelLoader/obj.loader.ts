...
import { IFace, IMaterialGroup, IModelData, IObjModel } from './ModelLoader.types';
...
const getAccObj = (path: string): IObjBuffer => ({ path, v: [], vt: [], vn: [], faces: [], materials: {}, material: '', mtllibs: [] });
...
const normalizeFaceIndex = (idx: number, type: 'v' | 'vt' | 'vn', acc: IObjBuffer) => {
	return idx > 0
		? idx - 1
		: acc[type].length + idx;
};
